const { createCanvas, Path2D, DOMMatrix } = require('@napi-rs/canvas');
const { AttachmentBuilder } = require('discord.js');

const DICE_SIZE = 50;
const MARGIN = 10;
const COLOR_BACKGROUND = 'black';
const COLOR_DICE = 'white';

module.exports = { rollRender }

async function rollRender(diceRoll) {
    const canvasWidth = diceRoll.diceAmount * (DICE_SIZE + MARGIN) + MARGIN;
    const canvasHeight = DICE_SIZE + 2 * MARGIN;
    const canvas = createCanvas(canvasWidth, canvasHeight);
    const context = canvas.getContext('2d');

    //fill background
    context.fillStyle = COLOR_BACKGROUND;
    context.fillRect(0, 0, canvasWidth, canvasHeight);

    for (let i in diceRoll.rolls) {
        renderDice(
            context, 
            W20_SHAPE, 
            diceRoll.rolls[i],
            MARGIN + i * (DICE_SIZE + MARGIN),
            MARGIN
        );
    }

    return new AttachmentBuilder(await canvas.encode('png'), { name: 'roll.png'});
}

async function renderDice(context, shape, number, x, y, color = '#ffffff') {
    context.setTransform(1, 0, 0, 1, 0, 0);

    context.fillStyle = color;
    context.translate(x, y);

    shape.forEach((path) => context.fill(path));

    context.font = '20px sans-serif';
    let textMeasure = context.measureText(number.toString());
    context.fillText(number.toString(), 25 - (textMeasure.width / 2), 33);

    /* context.strokeStyle = 'red';
    context.strokeRect(0, 0, 50, 50);
    context.strokeRect(0, 0, 25, 25);
    context.strokeRect(25, 25, 25, 25); */

    return context;
}

const W4_SHAPE = [
    new Path2D('M142.1,5.09c-3.2-3.66-7.99-5.53-12.82-5.01-4.83.53-9.11,3.38-11.44,7.65L1.82,219.68c-2.51,4.59-2.42,10.23.25,14.73s7.57,7.29,12.8,7.29h235.12c8.2,0,14.88-6.67,14.88-14.88v-75.5c0-3.6-1.3-7.07-3.67-9.78L142.1,5.09ZM254.94,226.82c0,2.73-2.22,4.96-4.96,4.96H14.87c-1.74,0-3.38-.93-4.27-2.43-.89-1.5-.92-3.38-.08-4.91L126.54,12.5c.78-1.42,2.21-2.38,3.82-2.55.18-.02.36-.03.54-.03,1.42,0,2.79.61,3.74,1.7l119.09,136.44c.79.9,1.22,2.06,1.22,3.26v75.5Z')
        .transform(new DOMMatrix([0.188772, 0, 0, 0.206863, 0.000331, -0.004476]),),
    new Path2D('M135.38,18.91s35.75,70.85,53.33,104.58c17.53,34.93,35.98,69.69,57.7,102.24-15.13-36.09-33.91-70.67-53.33-104.58-18.39-33.26-38.69-69.49-57.7-102.24Z')
        .transform(new DOMMatrix([0.188772, 0, 0, 0.206863, 0.000331, -0.004476]),),
];

const W6_SHAPE = [
    new Path2D('M208.6,0H31.1c-3.97,0-7.71,1.55-10.52,4.36L4.36,20.57C1.55,23.38,0,27.12,0,31.09v177.5c0,8.2,6.67,14.88,14.88,14.88h177.5c3.97,0,7.71-1.55,10.52-4.36l16.22-16.22c2.81-2.81,4.36-6.55,4.36-10.52V14.88c0-8.2-6.67-14.88-14.88-14.88ZM213.56,192.38c0,1.33-.52,2.57-1.45,3.51l-16.22,16.22c-.92.92-2.2,1.45-3.51,1.45H14.88c-2.73,0-4.96-2.23-4.96-4.96V31.09c0-1.32.52-2.57,1.45-3.51L27.59,11.37c.94-.94,2.18-1.45,3.51-1.45h177.5c2.74,0,4.96,2.23,4.96,4.96v177.5Z')
        .transform(new DOMMatrix([0.223734, 0, 0, 0.223744, 0, 0])),
    new Path2D('195.52 23.9 17.5 26.99 193.44 30.03 196.49 205.34 199.57 27.95 210.38 13.1 195.52 23.9')
        .transform(new DOMMatrix([0.223734, 0, 0, 0.223744, 0, 0])),
];

const W8_SHAPE = [
    new Path2D('M266.53,107.85L145.56,3.63c-5.63-4.85-13.8-4.85-19.42,0L5.17,107.85c-3.28,2.83-5.17,6.94-5.17,11.27v103.66c0,5.3,2.85,10.24,7.44,12.89l120.97,69.83c2.29,1.32,4.87,1.99,7.44,1.99s5.15-.66,7.44-1.99l120.97-69.83c4.59-2.65,7.44-7.59,7.44-12.89v-103.66c0-4.34-1.88-8.44-5.17-11.27ZM261.77,222.78c0,1.77-.95,3.41-2.48,4.3l-120.97,69.83c-1.53.88-3.43.88-4.96,0L12.4,227.08c-1.53-.88-2.48-2.53-2.48-4.29v-103.66c0-1.44.63-2.82,1.72-3.76L132.61,11.15c1.88-1.62,4.6-1.62,6.47,0l120.97,104.21c1.09.94,1.72,2.31,1.72,3.76v103.66Z')
        .transform(new DOMMatrix([0.184026, 0, 0, 0.162603, 0, -0.00628])),
    new Path2D('M23.23,228.51c74.72,3.42,150.52,3.39,225.24,0-74.72-3.39-150.52-3.42-225.24,0Z')
        .transform(new DOMMatrix([0.184026, 0, 0, 0.162603, 0, -0.00628])),
    new Path2D('M133.42,14.58c-20.19,32.55-41.74,68.5-61.33,101.58-20.63,33.68-40.63,68.07-57.03,104.05,22.88-32.26,42.56-66.82,61.32-101.58,18.76-33.52,39.04-70.28,57.03-104.05Z')
        .transform(new DOMMatrix([0.184026, 0, 0, 0.162603, 0, -0.00628])),
    new Path2D('M137.93,14.17c18.04,33.88,38.34,70.64,57.14,104.26,18.8,34.83,38.52,69.46,61.44,101.78-16.44-36.06-36.48-70.51-57.14-104.26-19.6-33.12-41.23-69.2-61.44-101.79Z')
        .transform(new DOMMatrix([0.184026, 0, 0, 0.162603, 0, -0.00628])),
];

const W10_SHAPE = [
    new Path2D('M148.37,311.98c-3.22,0-6.45-1.05-9.14-3.14L5.74,204.94c-3.65-2.84-5.74-7.12-5.74-11.74v-28.28c0-3.65,1.34-7.16,3.76-9.89L137.25,4.99c2.82-3.17,6.87-4.99,11.12-4.99s8.3,1.82,11.12,4.99l133.49,150.05c2.43,2.73,3.76,6.24,3.76,9.89v28.28c0,4.62-2.09,8.9-5.74,11.74l-133.49,103.89c-2.69,2.09-5.91,3.14-9.14,3.14ZM144.66,11.58L11.17,161.62c-.81.91-1.25,2.08-1.25,3.3v28.28c0,1.54.7,2.97,1.91,3.91l133.49,103.9c1.79,1.4,4.3,1.4,6.09,0l133.49-103.89c1.22-.95,1.91-2.37,1.91-3.91v-28.28c0-1.22-.45-2.39-1.25-3.3L152.07,11.58c-.94-1.06-2.29-1.66-3.71-1.66s-2.77.61-3.71,1.66Z')
        .transform(new DOMMatrix([0.168498, 0, 0, 0.160267, 0, 0])),
    new Path2D('M281.44,195.12l-42.68,10.18L151.37,15.01l82.01,193.31c.5,1.18,1.42,2.12,2.58,2.65,1.25.57,2.67.61,3.94.12l41.52-15.97Z')
        .transform(new DOMMatrix([0.168498, 0, 0, 0.160267, 0, 0])),
    new Path2D('148.37 254.74 71.74 215.58 145.42 260.06 148.37 296.85 151.32 260.06 226.23 214.89 148.37 254.74')
        .transform(new DOMMatrix([0.168498, 0, 0, 0.160267, 0, 0])),
    new Path2D('M145.17,15.37L57.96,205.31l-42.64-9.97,41.5,15.77c1.28.49,2.69.44,3.94-.13,1.16-.53,2.08-1.47,2.57-2.65L145.17,15.37Z')
        .transform(new DOMMatrix([0.168498, 0, 0, 0.160267, 0, 0])),
];

const W12_SHAPE = [
    new Path2D('M141.13,295.29c-1.55,0-3.1-.24-4.6-.73l-78.03-25.37c-3.01-.98-5.58-2.85-7.44-5.4L2.84,197.41C.98,194.86,0,191.83,0,188.67v-82.06C0,103.45.98,100.43,2.85,97.86L51.08,31.49c1.86-2.56,4.43-4.43,7.44-5.4L136.55.73c3.01-.98,6.19-.98,9.2,0l78.03,25.37c3.01.98,5.58,2.85,7.44,5.4l48.23,66.38c1.86,2.56,2.84,5.58,2.84,8.75v82.06c0,3.16-.99,6.18-2.85,8.75l-48.23,66.36c-1.86,2.56-4.43,4.43-7.44,5.4l-78.04,25.36c-1.5.49-3.05.73-4.6.73ZM141.15,9.92c-.52,0-1.03.08-1.53.24l-78.03,25.35c-1,.33-1.86.95-2.48,1.8L10.87,103.7c-.62.86-.95,1.87-.95,2.91v82.06c0,1.06.33,2.07.94,2.92l48.23,66.38c.62.85,1.48,1.48,2.48,1.8l78.03,25.37c1,.33,2.06.33,3.07,0l78.04-25.36c1-.33,1.86-.95,2.48-1.8l48.23-66.36c.61-.85.95-1.88.95-2.91v-82.06c0-1.06-.33-2.07-.94-2.92l-48.23-66.38c-.62-.85-1.48-1.48-2.48-1.8l-78.03-25.37c-.5-.16-1.02-.24-1.53-.24Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
    new Path2D('M196.31,209.57l31.93-86.47,40.72-16.98-43.09,10.24c-1.12.27-2.14.84-2.95,1.65-.75.75-1.3,1.68-1.58,2.7l-25.03,88.86Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
    new Path2D('M197.09,218.55c-.6-.98-1.46-1.78-2.48-2.3-.95-.48-2-.71-3.06-.67l-92.48,3.66,92.34,3.65,28.72,33.48-23.04-37.82Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
    new Path2D('M131.17,66.41l-74.9,49.89-42.94-10.2,40.89,17.05c1.06.44,2.22.58,3.35.4,1.05-.17,2.04-.6,2.87-1.26l70.72-55.88Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
    new Path2D('M59.93,129.35l25.18,89.35-23.53,38.45,29.42-34.39c.74-.87,1.23-1.93,1.41-3.06.17-1.05.06-2.13-.31-3.12l-32.17-87.22Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
    new Path2D('M137.58,57.41c-.09,1.14.13,2.29.66,3.31.48.95,1.2,1.75,2.08,2.34l75.63,50.44-71.23-56.27-3.57-43.99-3.57,44.16Z')
        .transform(new DOMMatrix([0.177123, 0, 0, 0.169322, 0, -0.004153])),
];

const W20_SHAPE = [
    new Path2D('M7.44,237.93l121.34,70.06c2.29,1.32,4.87,1.99,7.44,1.99s5.15-.66,7.44-1.99l121.34-70.06c4.59-2.65,7.44-7.59,7.44-12.89V84.93c0-5.3-2.85-10.24-7.44-12.89L143.67,1.99c-4.59-2.65-10.29-2.65-14.88,0L7.44,72.05C2.85,74.69,0,79.63,0,84.93v140.11c0,5.3,2.85,10.24,7.44,12.89ZM9.92,84.93c0-1.77.95-3.41,2.48-4.29L133.75,10.58c.77-.44,1.62-.66,2.48-.66s1.71.22,2.48.66l121.34,70.06c1.53.88,2.48,2.53,2.48,4.3v140.12c0,1.77-.95,3.41-2.48,4.29l-121.34,70.06c-1.53.88-3.43.88-4.96,0L12.4,229.34c-1.53-.88-2.48-2.53-2.48-4.3V84.93Z')
        .transform(new DOMMatrix([0.183527, 0, 0, 0.161302, 0, 0.002097])),
    new Path2D('M134.8,47.42l94.92,158.29s-70.13-126.27-87.16-156.92l113.31,32.1-116.77-39.42-2.88-29.54-2.88,29.56L13.64,82.92c.11-.04,103.67-30.37,121.16-35.5Z')
        .transform(new DOMMatrix([0.183527, 0, 0, 0.161302, 0, 0.002097])),
    new Path2D('M50.04,216.89c57.29,2.56,115.4,2.54,172.69,0-57.29-2.54-115.4-2.56-172.69,0Z')
        .transform(new DOMMatrix([0.183527, 0, 0, 0.161302, 0, 0.002097])),
    new Path2D('M35.6,220.29l99.34,76.01c-.09-.07-80.56-68.15-94.73-80.12L125.13,62.99S55.53,178.82,37.68,208.54L11.82,86.09l21.05,129.18-21.17,11.89,23.9-6.88Z')
        .transform(new DOMMatrix([0.183527, 0, 0, 0.161302, 0, 0.002097])),
    new Path2D('233.35 215.22 137.74 296.3 236.76 220.34 259.71 227.33 239.56 215.26 259.57 85.22 233.35 215.22')
        .transform(new DOMMatrix([0.183527, 0, 0, 0.161302, 0, 0.002097])),
];